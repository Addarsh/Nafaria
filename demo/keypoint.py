# Copied and modified from MaskRCNN keypoint file.
# Modification is just the addition of Necklace Keypoints class.
import torch
...
class NecklaceKeypoints(Keypoints):
    NAMES = [
        'left_hook',
        'center',
        'right_hook',
...
# TODO this doesn't look great
NecklaceKeypoints.FLIP_INDS = _create_flip_indices(NecklaceKeypoints.NAMES, NecklaceKeypoints.FLIP_MAP)
def kp_connections(keypoints):
    kp_lines = [
        [keypoints.index('left_hook'), keypoints.index('center')],
        [keypoints.index('center'), keypoints.index('right_hook')],
    ]
    return kp_lines
NecklaceKeypoints.CONNECTIONS = kp_connections(NecklaceKeypoints.NAMES)
